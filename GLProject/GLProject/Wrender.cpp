...
Vec3D<float> CWrender::Calculate_Normal(CKH_NORM_DATA fdata, CKH_NORM_DATA ldata)
{
	ckh_vec3d.x = fdata.x;
	ckh_vec3d.y = fdata.y;
	ckh_vec3d.z = fdata.z;
...
void CWrender::MakeBrickTexture_FUNC()
{
	glGenTextures(m_texture_num, m_textures);
	//----------------------------------------------------------------------------------------------------------------- 이걸 여러번 반복 ->텍스처가 여러개일시
	glBindTexture(GL_TEXTURE_2D, m_textures[0]);
	m_TexBits = get_load->ckh_LoadBitmap("steal.bmp", &m_info);
